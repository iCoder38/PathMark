...
class ride_status: UIViewController , CLLocationManagerDelegate , MKMapViewDelegate, UITextFieldDelegate {
    ...
    func fetchRoute(from start: CLLocationCoordinate2D, to end: CLLocationCoordinate2D) {
        ...
        let urlString = "https://maps.googleapis.com/maps/api/directions/json?origin=\(origin)&destination=\(destination)&traffic_model=best_guess&departure_time=now&key=\(apiKey)"
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                   ...
                   let points = overviewPolyline["points"] as? String {
                    ...
                    print(json)
                    print(route["legs"] as Any)
                    
                    let duration2 = route["legs"] as? [[String: Any]]
                    let duration1 = duration2?.first
                     
                    let DIT = duration1!["duration_in_traffic"] as? [String: Any]
                    print(DIT?.first as Any)
                    
                    let duration = route["duration"] as? [String: Any]
                    let durationInTraffic = route["duration_in_traffic"] as? [String: Any]
                    print(duration as Any)
                    print(durationInTraffic as Any)
                    
                    let totalDurationWithoutTraffic = route["legs"] as? Int
                    let totalDurationWithTraffic = 4000  // Duration in seconds with traffic (total route)

//                    let routeSteps: [RouteStep] = getRouteSteps()  // Fetch route steps (duration, distance, etc.)

                    // Calculate the ratio of traffic increase for the entire route
//                    let trafficRatio = totalDurationWithTraffic / totalDurationWithoutTraffic
